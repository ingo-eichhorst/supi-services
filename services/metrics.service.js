  ...
  methods: {
    printRequest (id) {
      ...
      let printSpanTime = (span) => {
        ...
        if (actionName.length > maxActionName) { actionName = _.truncate(span.action.name, { length: maxActionName }) }
        ...
        if (span.spans.length > 0) { span.spans.forEach(spanID => printSpanTime(this.requests[spanID])) }
      }
